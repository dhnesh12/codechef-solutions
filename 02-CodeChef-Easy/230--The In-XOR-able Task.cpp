...
int main(){
    ...
    loop(erer2,1,erer+1){
        ...
        loop(i,1,n+1){
            ...
            loop(j,0,31){
                if(j==len) msb[i][len]=msb[i-1][len]+1;
                else msb[i][j]=msb[i-1][j];
            }
        }
        // loop(i,0,n+1){
        //     loop(j,0,10){
        ...
        loop(o,0,q){
            lli l, r;
            cin>>l>>r;
            lli ans=(z[r]-z[l-1])*(r-l+1);
            loop(i,0,31){
                lli a = msb[r][i]-msb[l-1][i];
                lli b = sb[r][i]-sb[l-1][i];
                ans+=(a*(b-a));
            }
            cout<<ans<<'\n';
        }
