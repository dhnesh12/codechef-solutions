...
import java.text.DecimalFormat;
	...
	/*	 boolean prime[]=new boolean[10000000];
			...
			{  
				 int n = in.nextInt();
		            ...
		            for(int i=0; i<q; i++)
		            {
		                ...
		                int idx = pos - 1;
		                int presentval = arr[idx];

		                if(idx-1>=0 && presentval != arr[idx-1]) ans--;
		                if(idx+1<n && presentval != arr[idx+1]) ans--;

		                if(idx-1>=0 && newval != arr[idx-1]) ans++;
		                if(idx+1<n && newval != arr[idx+1]) ans++;
	    ...
	    public static int lenOfGoodSubseq(int[] arr)
	    {
	        int ans = 1;
