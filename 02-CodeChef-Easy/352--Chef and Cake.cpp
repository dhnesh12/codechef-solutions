...
void solve()
{
    ll n, k, q;
    cin >> n >> k >> q;
    ll a, b, c, d, e, f, r, s, t, m, p;
    cin >> a >> b >> c >> d >> e >> f >> r >> s >> t >> m >> p;
...
//    printVector(arr);
    ...
    for (ll i = 1; i <= k; i++)
    {
        ...
        {
            while (!dq.empty() && dq.back().fi > arr[i])
                dq.pop_back();
            dq.push_back({arr[i], i});
        }
    }
    ...
    for (ll i = 2; i <= n; i++)
    {
        if (i + k - 1 <= n)
        {
            while (!dq.empty() && (dq.back().fi > arr[i + k - 1] || dq.back().se < i))
                dq.pop_back();
            while (!dq.empty() && dq.front().se < i)
                dq.pop_front();
...
//    printArray(dp, n);
    ...
    for (ll i = 0; i < q; i++)
    {
        ...
        le = l1 + 1;
        ri = min(le + k - 1 + d1, n);
        ll mini = min(dp[le], dp[ri - k + 1]);
        sum += mini;
        prod = multiply(prod, mini, MOD);
    }
    cout << sum << " " << prod << endl;
}
int main()
