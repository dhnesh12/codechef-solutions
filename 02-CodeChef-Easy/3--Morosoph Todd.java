...
class Solver {
    ...
    void solve(int tc) throws Exception {
        ...
        kbinLen = Long.toBinaryString(temp).length();
        K = new BitSet();
        for (i = 0; i < 62; ++i) {
            if (temp % 2 == 1) K.set(i);
            temp >>= 1;
        ...
        ans = false;
        dfs(0, -7, Integer.MAX_VALUE >> 2, new BitSet(), new BitSet(), true);
        if (ans) hp.println("YES");
        else hp.println("NO");
    }
    ...
    int kbinLen;
    boolean ans;
    void dfs(int node, int par, int minSoFar, BitSet xorSoFar, BitSet changable, boolean allSame) {
        if (ans) return;
        graph[node].remove((Object) par);

        minSoFar = Math.min(minSoFar, P[node]);
        xorSoFar.flip(P[node] - 1);
        changable.set(0, P[node] - 1);
        if (par >= 0 && P[node] != P[par]) allSame = false;

        //System.err.println(node + " --> " + minSoFar + " " + xorSoFar + " " + changable);

        if (graph[node].isEmpty()) {
            // check if K can be formed using AND
            if (minSoFar > kbinLen && !allSame) {
                ans = true;
                //System.err.println(node + " using AND");
            } else if (minSoFar == kbinLen) {
                ans = true;
                //System.err.println(node + " using AND");
            }

            // check is K can be formed using XOR
            boolean ok = true;
            for (int i = 0; i < BITLEN && ok; ++i) if (!changable.get(i)) {
                if (K.get(i) != xorSoFar.get(i)) ok = false;
            }
            if (ok) {
                ans = true;
                //System.err.println(node + " using XOR");
            }
        } else {
            for (int itr : graph[node]) {
                dfs(itr, node, minSoFar, (BitSet) xorSoFar.clone(), (BitSet) changable.clone(), allSame);
            }
        }
    }
}
