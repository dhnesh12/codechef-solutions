...
template <uint64_t Mod = 100'000'000'003> 
struct Modular {
    using M = Modular;
    uint64_t v;
...
struct Elem{
    ...
    mint v;
    Elem(){}
    Elem(ll x, ll y, mint v): x(x), y(y), v(v){}
};

vector<mint> MatrixRecurrence(int n, vector<Elem> M) {
    vector<mint> rd1, rd2;
    FER(i, 0, n){
        rd1.pb(mint(rand_int(1, mody-1)));
        rd2.pb(mint(rand_int(1, mody-1)));
    }
    vector<mint> v;
    FER(i, 0, ((n+1)<<1)){
        mint tmp = 0;
        FER(j, 0, n) tmp += rd2[j] * rd1[j];
        v.pb(tmp);
        vector<mint> nxt(n);
...
inline mint Determinant(ll n, vector<Elem> M){
    vector<mint> cur;
    FER(i, 0, n) cur.pb(mint(rand_int(1, mody-1)));
    for(auto &xd: M) xd.v *=cur[xd.y];
    auto ans = MatrixRecurrence(n, M);
    mint ret = ans.back(), ket = mint(0);
    for(auto xd: cur) ret /= xd;
    ket -= mint(1);
    if(n&1) ret *= ket;
    return ret;
}
...
namespace ntt {
    ...
    //for 1e9+7 ntt
    constexpr uint64_t mod = 100000000003;
...
int main(){
	...
	while(t--){
		...
		vector<mint> base;
		cur -= mint(1);
		FER(i, 0, n) base.pb(cur);
		FER(i, 0, n) mat.pb(base);
		cur = mint(n);
		cur *= mint(k);
		cur -= mint(1);
		FER(i, 0, n) mat[i][i] = cur;
		FER(i, 0, m){
			ll a, b; read(a), read(b); a--, b--;
			mat[a][b] = mat[b][a] = mint(0);
			mat[a][a] -= mint(1);
			mat[b][b] -= mint(1);
		}
		FER(i, 0, n) FER(j, 0, n) mat[i][j] += mint(1);
		FER(i, 0, n) FER(j, 0, n) v.pb(Elem(i, j, mat[i][j]));
		auto ans = Determinant(n, v);
		auto ret = mint(n*k).pow(2);
		auto jael = ans.pow(k) * ret.inv();
		cout<<jael<<"\n";
	}    
    return 0;
}