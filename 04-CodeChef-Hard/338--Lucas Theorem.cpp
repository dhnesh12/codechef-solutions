...
//        pairfft(fa, fb);
    ...
    vector<LL> anyMod(const vector<LL> &a, const vector<LL> &b, int P) {
        int M = P, B = sqrt(M)+1;
        int n = 1;
        while (n < a.size()+ b.size())  n<<=1;
...
int n, P;
vector<LL> mul(vector<vector<LL>> &p, int l, int r) {
    if (l == r) return p[l];
    int mid = (l+r)/2;
    vector<LL> ll = mul(p, l, mid), rr = mul(p, mid+1, r);

    ll = FFT::anyMod(ll, rr, P);
    while (ll.size() && ll.back() == 0) ll.pop_back();
    return ll;
}
...
int main() {
    ...
    while (t--) {
        ...
        for (int i=1; i<=n; i++) polys.push_back({i%P, 1});
        vector<LL> sol = mul(polys, 0, n-1);
        LL ans = 0;
        for (LL x: sol)
            if (x%P)    ans++;
        cout<<ans<<endl;
