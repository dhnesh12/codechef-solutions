...
struct cht {
    ...
    void erase_line(spit it){
        ...
        ///inter.erase({intersection(it),*it});
        spit prv=get_prv(it);

        if(prv!=lines.end()){
            /*inter.erase(inter.find({intersection(prv),*prv}));
            inter.insert({intersection(prv,get_nxt(it)),*prv});*/
            intersection(prv,get_nxt(it));
        }
    ...
    spit ins_line(pii x){
        ...
        if(prv!=lines.end()){
            ///inter.erase(inter.find( {intersection(prv,get_nxt(it)),*prv} ));
            ///inter.insert({intersection(prv),*prv});
            intersection(prv);
        }

        ///inter.insert({intersection(it),*it});
        intersection(it);
