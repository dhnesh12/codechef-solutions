...
int main() {
  ...
  for (long long p = 1; p * p <= k; p++) {
    long long from = k / ((p + 1) * (p + 1)) + 1;
    long long to = k / (p * p);
    if (from <= to) {
      long long cc = 0;
      for (long long i = 1; i * i <= to; i++) {
        cc -= mo[i] * (to / (i * i) - (from - 1) / (i * i));
      }
      if (cc > 0) {
        v.emplace_back(cc, p);
      }
  ...
  debug(clock());
//  debug(v);
  function<pair<vector<Mint>, vector<Mint>>(int, int)> Go = [&](int from, int to) {
    if (from == to) {
      return make_pair(vector<Mint> {v[from].first}, vector<Mint> {1, -v[from].second});
    }
    int mid = (from + to) >> 1;
    auto x = Go(from, mid);
    auto y = Go(mid + 1, to);
  ...
  auto aux = Go(0, (int) v.size() - 1);
  aux.first.resize(MAX);
  aux.second.resize(MAX);
  auto prod = aux.first * inverse(aux.second);
  prod.resize(MAX);
  debug(clock());
