...
public:
  ...
  void deleteWord(string word)
  {
    ...
    for(int i=0;i<n;i++)
    {
      ...
      if(curr->child[index]==NULL)
      {
        throw "Invalid deletion Exception";
      }

      curr=curr->child[index];
      if(curr->count<=0) throw "Invalid deletion Exception";
      else curr->count--;
    }

    if(curr->endsHere<=0) throw "Invalid deletion Exception";
    else curr->endsHere--;
...
ll dfs(auto root,ll &score,ll level)
{
  ll sub=0;
  for(int i=0;i<26;i++)
  {
    if(root->child[i])
    {
      sub+=dfs(root->child[i],score,level+1);
    }
  }

  if(level!=0)
  score+=(level/2)*(level/2)*((root->count-sub)/2);
  return ((root->count)/2)*2;
}
...
int main()
{
  ...
  while(t--)
  {
    ...
    for(int i=0;i<n;i++)
    {
      ...
      Dict.insert(modified);
    }
    ll score=0;
    dfs(Dict.root,score,0);
    cout<<score<<endl;
  }
