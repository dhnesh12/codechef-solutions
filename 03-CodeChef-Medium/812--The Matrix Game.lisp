...
(defun main ()
  (let* ((tt (read)))
    (dotimes (_ tt)
      (let* ((n (read))
             (m (read))
             (as (make-array (list n m) :element-type 'uint8 :initial-element 0)))
        (dotimes (i n)
          (dotimes (j m)
            (setf (aref as i j) (read-fixnum))))
        (labels ((calc (row)
                   (loop with res = (aref as row (- m 1))
                         for j from (- m 2) downto 0
                         for a = (aref as row j)
                         when (<= a res)
                         do (setq res (- a 1))
                         else
...
(progn
  ...
  (defparameter *problem-url* "https://www.codechef.com/problems/SNCK01"))
