    ...
    (write-string
     (with-output-to-string (*standard-output* nil :element-type 'base-char)
       (dotimes (_ tt)
         (let* ((n (read))
                (m (read))
                (xs (make-array m :element-type 'uint31 :initial-element 0))
           ...
           (dotimes (i m)
             (let ((x (- (read-fixnum) 1))
               ...
               (setf (aref xs i) x
                     (aref ys i) y)))
           (write-line (cond ((<= m 1) "YES")
                             ((= m 2)
                              (if (or (= (aref xs 0) (aref xs 1))
                                      (= (aref xs 0) (aref ys 1))
                                      (= (aref ys 0) (aref xs 1))
                                      (= (aref ys 0) (aref ys 1)))
                                  "YES"
...
(progn
  ...
  (defparameter *problem-url* "https://www.codechef.com/problems/WSC"))
