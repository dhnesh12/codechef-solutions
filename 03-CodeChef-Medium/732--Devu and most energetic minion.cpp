...
vector<ll>lft[mx],rgt[mx];

void left(int i,int j,int prefix)
{
    if(i+j>n-1)
        return ;
    if(i+j==n-1)
    {
        lft[i].emplace_back(prefix^arr[i][j]) ;
    }
    else
    {
        left(i+1,j,prefix^arr[i][j]) ;
        left(i,j+1,prefix^arr[i][j]) ;
    }

}
void right(int i,int j,int prefix)
{
    if(i+j<n-1)
        return ;
    if(i+j==n-1)
    {
        rgt[i].emplace_back(prefix) ;
    }
    else
    {
        right(i-1,j,prefix^arr[i][j]) ;
        right(i,j-1,prefix^arr[i][j]) ;
    }
}
...
int main()
{
    ...
    f0(i,n)
    {
      ...
      //  dbg(rgt[i]);
        ...
        f0(j,rgt[i].sz)
        {
            best = max(best,query(root,rgt[i][j]));
        }
    }
    cout << best << endl;
