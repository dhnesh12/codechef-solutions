...
template <typename T>
vector<Modular<T>> multiply_all(const vector<vector<Modular<T>>> &polynomials) {
...
vector<Mint> generate_G(int l, int r) {
    if(l == r) {
        return vector<Mint>({a[l] + 1, -2});
    }
    int m = (l + r) >> 1;
    return generate_G(l, m) * generate_G(m + 1, r);
}
